	...
	const getSidebarContent = (): MenuItem[] => {
		...
		const hasModuleAccess = (module: ModuleKey) => {
			...
			return permissions?.some(permission => {
				if (module === "isAgent") {
					return permission?.isAgent === true;
				}
				if (module === "isTalentPool") {
					return permission?.isTalentPool === true;
				}
				const mod = permission.modules?.[module];
					if (!mod) return false;

		// At least one action (read/create/update/delete) must be true
		...
		const commonMenuItems = {
			collectionList: {
				...
				module: "collection" as ModuleKey
			},
			agentList: {
				name: "Agent List",
			...
			talentPool: {
				...
				id: uniqueId(),
				url: "/talent-pool",
				module: "isTalentPool" as ModuleKey
			},
			jobPosting: {
				name: "Job Posting",
