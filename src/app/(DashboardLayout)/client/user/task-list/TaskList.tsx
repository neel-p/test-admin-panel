...
import DynamicForm from '../view-user/DynamicForm'
...
const ViewTaskModal = ({ task, onClose }: { task: TaskData | null, onClose: () => void }) => {
	...
	const renderField = (label: string, value: any, className: string = "") => {
		...
		return (
			<div className={`flex flex-col ${className} p-4 bg-white rounded-lg border border-gray-200`} key={uniqueId()}>
				<Label className="text-gray-600 font-medium mb-1">{label}</Label>
	...
	const renderArrayField = (label: string, items: any[] = [], renderItem: (item: any, index: number) => React.ReactNode) => {
		...
		return (
			<div className="p-4 bg-white rounded-lg border border-gray-200"  key={uniqueId()}>
				<Label className="text-gray-600 font-medium mb-1">{label}</Label>
	...
	const renderContent = () => {
		...
		const filteredInput = Object.entries(task.input || {}).filter(([key]) => 
			!['jd', 'JD', 'detail', 'DETAIL', 'outbound_result'].includes(key.toLowerCase())
		);
					...
					<Card className="prose prose-sm max-w-none">
						...
						<div className="grid grid-cols-1 md:grid-cols-1 gap-4">
							{Object.entries(task.output)
								.filter(([key]) => !['id', 'createdAt', 'updatedAt', 'taskId', 'status', 'outbound_result'].includes(key))
								.map(([key, value]) => {
											...
											<div key={key} className="p-4 bg-white rounded-lg border border-gray-200">
												...
												<div className="pl-4 border-l-4 border-primary">
													{Object.entries(value)
														.filter(([subKey]) => !['id', 'createdAt', 'updatedAt'].includes(subKey))
														.map(([subKey, subValue]) => (
...
const TaskList = () => {
	...
	const searchParamsData = searchParams?.get("data") || null;
	...
	const handleOutBound = async (id: string) => {
		try {
			const payload = {
				body: {
					task_id: parseInt(id),
					client_id: parseInt(decodedData?.clientId)
				},
			};
		...
		try {
			const payload = {
				body: {
					...
					sortBy,
					sortOrder,
					userId: parseInt(decodedData?.id),
				},
			};
			...
			<DeleteWarningModal
				...
				onConfirm={() => {
					if (isDeleteModalOpen.data?.id) {
						handleDeleteClientUser(isDeleteModalOpen.data.id);
					}
				}}
				title={`Delete Job?`}
